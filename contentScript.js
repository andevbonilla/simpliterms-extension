          ...
          const requestSummaryInfo = async(tokenValidator, posibleWords, politicsType) => {
            ...
            for (const tag of linksTag) {
                ...
                for (const option of posibleWords) {
                  ...
                  if (tag.textContent.toString().replaceAll(' ','').toLowerCase().includes(option.toString().replaceAll(' ','').toLowerCase())) {
                    
                    if (!politicsURLs.includes(tag.getAttribute("href"))) {

                      const regexpUrlComplete = /^(http:|https:)/i;

                      if (regexpUrlComplete.test(tag.getAttribute("href").toString().trim())) {
